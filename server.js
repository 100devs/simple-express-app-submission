...
app.get('/api/:alienName', (req, res) => //the colon alienName represents that this is a query parameter. Hey express, this is a parameter. Treat it as such.
{
    const aliensName = req.params.alienName.toLowerCase();
    if(aliens[aliensName]){ //see if aliensName is in the aliens object, returns bool. this is called if(object[key])
...
app.listen(process.env.PORT || PORT, () => { //the process.env means Heroku can pick a different or better PORT if they need to (gives flexibility)
