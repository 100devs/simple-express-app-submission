...
app.get('/',(request, response)=>{
    db.collection('beaches').find().sort({likes: -1}).toArray()
    .then(data => {
...
app.delete('/deleteBeach', (request, response) => {
    db.collection('beaches').deleteOne({beachName: request.body.beachName})
    .then(result => {
        console.log('Beach Deleted')
        response.json('Beach Deleted')
    })
    .catch(error => console.error(error))
