...
function reloadTask(task) {
    ...
    if (stateEl) {
        ...
        if (task.state === "Fullfilled") {
            if (trashIcon?.classList.contains('pending'))
                trashIcon.classList.remove('pending');
            if (!trashIcon?.classList.contains('fullfilled'))
                trashIcon?.classList.add('fullfilled');
        }
        else {
            if (trashIcon?.classList.contains('fullfilled'))
                trashIcon.classList.remove('fullfilled');
            if (!trashIcon?.classList.contains('pending'))
                trashIcon?.classList.add('pending');
        }
    }
}
...
async function getTodosList() {
    var data = await fetch('api/todo')
        ...
        .then((data) => data ? populateTodosList(data) : console.log('No task in the list '));
}
(() => {
    // fetch data on load ?
    getTodosList();
